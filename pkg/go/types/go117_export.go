...
func init() {
	gossa.RegisterPackage(&gossa.Package{
		...
		NamedTypes: map[string]gossa.NamedType{
			...
			"Checker":       {reflect.TypeOf((*q.Checker)(nil)).Elem(), "", "Files,addDeclDep,allowVersion,anyType,applyTypeFunc,arguments,arityMatch,arrayLength,assertableTo,assignVar,assignVars,assignment,binary,blockBranches,builtin,callExpr,caseTypes,caseValues,checkFiles,closeScope,collectMethods,collectObjects,collectParams,collectTypeConstraints,collectTypeParams,comparison,completeInterface,consolidateMultiples,constDecl,conversion,convertUntyped,cycle,cycleError,declStmt,declare,declareInSet,declarePkgObj,declareTypeParams,definedType,dump,err,error,errorUnusedPkg,errorf,exclude,expr,exprInternal,exprList,exprOrType,exprWithHint,filename,funcBody,funcDecl,funcInst,funcType,genericType,handleBailout,ident,identical,identical0,identicalIgnoreTags,identicalTParams,implicitTypeAndValue,importPackage,index,indexExpr,indexedElts,infer,inferB,initConst,initFiles,initOrder,initVar,initVars,instantiate,instantiatedOperand,instantiatedType,interfaceType,invalidAST,invalidArg,invalidConversion,invalidOp,isTerminating,isTerminatingList,isTerminatingSwitch,isValidIndex,labels,langCompat,later,lookupFieldOrMethod,lookupType,markImports,missingMethod,multiExpr,multipleDefaults,newError,newErrorf,newNamed,newTypeParam,objDecl,op,openScope,ordinaryType,overflow,packageObjects,pop,processDelayed,push,qualifier,rawExpr,rawLookupFieldOrMethod,record,recordBuiltinType,recordCommaOkTypes,recordDef,recordImplicit,recordInferred,recordScope,recordSelection,recordTypeAndValue,recordUntyped,recordUse,rememberUntyped,reportAltDecl,reportCycle,representable,representation,resolveBaseTypeName,selector,shift,shortVarDecl,simpleStmt,singleIndex,singleValue,sliceExpr,softErrorf,sprintf,stmt,stmtList,structType,structuralType,subst,suspendedCall,tag,trace,typ,typInternal,typeAssertion,typeDecl,typeList,typeOrNil,unary,unpackRecv,unusedImports,updateExprType,updateExprVal,usage,use,useLHS,validType,varDecl,varType,walkDecl,walkDecls"},
			...
			"Interface":     {reflect.TypeOf((*q.Interface)(nil)).Elem(), "", "Complete,Embedded,EmbeddedType,Empty,ExplicitMethod,Method,NumEmbeddeds,NumExplicitMethods,NumMethods,String,Underlying,_HasTypeList,_IsComparable,_IsConstraint,assertCompleteness,is,isSatisfiedBy,iterate"},
			...
			"Named":         {reflect.TypeOf((*q.Named)(nil)).Elem(), "", "AddMethod,Method,NumMethods,Obj,SetUnderlying,String,Underlying,_SetTArgs,_TArgs,_TParams,setUnderlying,under"},
			...
			"Scope":         {reflect.TypeOf((*q.Scope)(nil)).Elem(), "", "Child,Contains,End,Innermost,Insert,Len,Lookup,LookupParent,Names,NumChildren,Parent,Pos,String,WriteTo,squash"},
			...
			"Signature":     {reflect.TypeOf((*q.Signature)(nil)).Elem(), "", "Params,Recv,Results,String,Underlying,Variadic,_SetTParams,_TParams"},
