...
func runtimeCallers(fr *frame, skip int, pc []uintptr) int {
	...
	for _, pc := range pcs {
		...
		if fn := findFuncByPC(fr.pfn.Interp, int(pc)); fn != nil && isWraperFuncName(fn.Fn.String()) {
			continue
		}
		rpc = append(rpc, pc)
	}
...
func isWraperFuncName(name string) bool {
...
func fixedFuncName(name string) (string, bool) {
...
func runtimeFunc(pfn *function) *runtime.Func {
	...
	var autogen bool
	f.name, autogen = fixedFuncName(fn.String())
	if autogen {
